...
public class MyImageView extends android.support.v7.widget.AppCompatImageView {
	...
	private void arithScaleRate() {
		...
		//横屏
			scaleHeight= PhotoShowActivity.screenWidth / (float) imageWidth;
			scaleWidth = PhotoShowActivity.screenHeight / (float) imageHeight;
		}else if(ori == mConfiguration.ORIENTATION_PORTRAIT){

		//竖屏
			scaleWidth = PhotoShowActivity.screenWidth / (float) imageWidth;
			scaleHeight = PhotoShowActivity.screenHeight / (float) imageHeight;
		}*/
		float scaleWidth = PhotoShowActivity.screenWidth / (float) imageWidth;
		float scaleHeight = PhotoShowActivity.screenHeight / (float) imageHeight;
		scaleRate = Math.min(scaleWidth, scaleHeight);//取小的，使得屏幕能够将图片完整显示
	}
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		//缩放到屏幕大小
		zoomTo(scaleRate,PhotoShowActivity.screenWidth / 2f, PhotoShowActivity.screenHeight / 2f);
...
//		RectF rect = new RectF(0, 0, imageWidth*getScale(), imageHeight*getScale());
	...
	public void layoutToCenter()
	{
		...
		//空白区域宽高
		float fill_width = PhotoShowActivity.screenWidth - width;
		float fill_height = PhotoShowActivity.screenHeight - height;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom =( PhotoShowActivity.screenWidth/2f)/imageWidth;
	...
	protected void onDraw(Canvas canvas) {
		...
		float width = imageWidth*getScale();
		float height = imageHeight*getScale();
		if (width > PhotoShowActivity.screenWidth) {
			// 如果图宽大于屏宽，就不用水平居中
			center(false, true);
			} else {
			center(true, true);
