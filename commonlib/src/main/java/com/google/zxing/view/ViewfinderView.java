...
//    canvas.drawText(labelText, frame.left + frame.width() / 2, frame.top - CORNER_RECT_HEIGHT, paint);
    ...
    textPaint.setTextSize(TwsTools.dip2px(this.getContext(),labelTextSize));
    textPaint.setColor(labelTextColor);
    String data = labelText;
//getWidth()表示绘制多宽后换行
    StaticLayout sl = new StaticLayout(data,textPaint,frame.width(), Layout.Alignment.ALIGN_CENTER,1.0f,0.0f,true);
//从0,0开始绘制

    canvas.translate(frame.left, frame.top - sl.getHeight() - 10);
    sl.draw(canvas);
    canvas.restore();
  }
